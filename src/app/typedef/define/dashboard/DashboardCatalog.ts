 ...
 *
 * 两份清单的**口径与归属校验逐字一致**（后端按根空间的子树取，见 `DashboardCatalogService`）：
 ...
 *
 * `did` 为空的条目丢掉（`CatalogDevice` 里它是主键，空的不该出现在任何下拉里）。
 * **`online` 故意不拷**：清单是进编辑态时取一次的，到对话框打开时那个状态可能已经过期 ——
 * 拷进来只会诱人拿它当实时状态用。
 */
export function catalogDevices(catalog: DashboardCatalog | null): DeviceEntity[] {
