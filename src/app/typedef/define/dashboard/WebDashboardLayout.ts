 ...
 *
 * 缺口是故意的：**没有 6 宽的 416**（细高条放图表没法看，那一竖条用矮卡摞就能填满），
 * **没有整宽的 92**（一行高的整宽卡没有对应的内容）。6 宽的 308 是后补的：它正好是
 * 「三张统计卡竖着叠起来」那一条（`92 + 16 + 92 + 16 + 92`），窄饼图也放得下。
 */
export const WIDGET_SIZES: Record<WidgetSize, { w: number; h: number }> = {
  ...
  W6H308: { w: 6, h: 3 }, // 308px，窄而高：饼图与「三张统计卡叠起来」的那一竖条
...
/**
 * 自动刷新间隔的可选项（秒），工具条那个下拉的选项表。**`0` = 不自动刷新**。
 *
 * 这是**整屏一个**的档位，不再挂在每张卡上（修订（十七））：一个页面上的读数该多久更新一次，
 * 是「我在怎么看这一屏」的问题，不是「这张卡是什么」的问题 —— 原先每张卡各带一个
 * `refresh`，用户配四张卡就要配四次，而配错的那张会一直安静地按错的周期请求。
 *
 * 档位从 30 秒到 1 小时：30 秒是「盯着看」的下限（再密就是在自己压后端），
 * 1 小时是「挂在大屏上」的上限。中间取 `1m` / `5m` / `15m` —— Grafana 的默认档位也是这几个，
 * 而开发机上最常用的 `5s` 没有放进来：这一屏最多九张卡、每张都要打一次统计查询，
 * 5 秒一轮对后端不划算。
 */
export const REFRESH_INTERVALS = [0, 30, 60, 300, 900, 3600];

/** 打开页面时的缺省间隔（秒）＝ `1m`。选它是因为它与改造前每张卡缺省的 60 秒一致 */
