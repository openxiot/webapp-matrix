...
import { CANVAS_PAD, NODE_H, NODE_W, Offsets, PANEL_GAP, PANEL_W, TreeLayout } from './project1.layout';
import { Project1Component } from './project1.component';

/*
 * 这一份是**挂载用例**（`TestBed.createComponent`），与 `project1.graph.spec.ts`（树拼得对不对）、
 * `project1.layout.spec.ts`（坐标算得对不对）互补：那两份钉的是纯函数，这一份钉的是
 * **这个页面跑不跑得起来、接线接对了没有**。
 *
 ...
 * 而那正是最该被钉住的一类。
 *
 * 拖动本身的**坐标**口径（祖先累加、画布跟随）在 `project1.layout.spec.ts` 里逐条钉着，
...
describe('Project1Component（挂载）', () => {
  ...
  async function mount(): Promise<ComponentFixture<Project1Component>> {
    const fixture = TestBed.createComponent(Project1Component);
  ...
  const inner = (fixture: ComponentFixture<Project1Component>): Internals =>
   ...
   */
  function drag(fixture: ComponentFixture<Project1Component>, key: string, dx: number, dy: number): void {
  ...
  function dragPanel(fixture: ComponentFixture<Project1Component>, dx: number, dy: number): void {
    const api = inner(fixture);
   ...
   */
  function posOf(fixture: ComponentFixture<Project1Component>, key: string): { x: number; y: number } {
   ...
   */
  function cardOf(fixture: ComponentFixture<Project1Component>, key: string): HTMLElement {
   ...
   * 这是这一轮的主角：拖父卡片，**整支跟着走**。
   *
   * 坐标怎么累加在 `project1.layout.spec.ts` 里逐条钉着，这里钉的是**接线**：
  ...
  /*
   * 卡片不许被拖出画板。画布原点是固定的（见 `project1.layout.ts` 的文件头），
  ...
  /* ── 选中与信息框 ─────────────────────────────────────────────────────────
   *
   * 落位那套几何（贴右 / 翻左 / 两边界）在 `project1.layout.spec.ts` 里逐条钉着，
  ...
  function click(fixture: ComponentFixture<Project1Component>, el: HTMLElement): void {
    el.click();
    fixture.detectChanges();
  }

  function panelOf(fixture: ComponentFixture<Project1Component>): HTMLElement | null {
   ...
   *
   * 落位那几条**纯函数**的口径在 `project1.layout.spec.ts` 里逐条钉着（下面有多少地方、
  ...
  /*
   * 「点操作按钮，进入对应的功能实现」这条要求的落点：那几颗按钮**不是**这一页
   * 自己实现的，它们是父类 `ProjectComponent` 上那一批。所以这里验的是
   * **接线接对了**，不是「删干净了」——
   *
   * 删除走的是确认框（`NzModalService.create`），桩里给的 `afterClose: of(true)`
   * 等于用户点了「确认」，于是同步落到 `deleteSpace` 上。用到的 id 是**框里那张卡**
   * 的 id（`sp2`，一个没有子空间的空间 —— 有子空间的那条路会提前被父类拦下弹警告），
   * 传错了这条就红。
   */
  /*
   * 框里那几颗写操作按钮点下去，进的是**父类** `ProjectComponent` 上的实现 ——
   * 连同它的确认对话框、提示文案、删完重新取数，不是这一页重写的一套。
   * 这正是「可以参考 ProjectComponent 组件页面」那句话的落点：功能是同一个功能，
   * 这一页只负责在卡片旁边把它露出来。哪天有人在子类里覆盖了一个同名方法
   * （哪怕只是改个提示），这条就红了 —— 那正是该被看见的一次分叉。
   */
  it('框里的写操作接的是父类那一份实现，这一页没有另抄一遍', async () => {
    const fixture = await mount();
    const inst = fixture.componentInstance as unknown as Record<string, unknown>;

    expect(inst['removeSpace']).toBe(ProjectComponent.prototype['removeSpace']);
    expect(inst['addDevice']).toBe(ProjectComponent.prototype['addDevice']);
    expect(inst['addChildSpace']).toBe(ProjectComponent.prototype['addChildSpace']);
    expect(inst['removeDevice']).toBe(ProjectComponent.prototype['removeDevice']);
    expect(inst['removeService']).toBe(ProjectComponent.prototype['removeService']);
    // 只读的那两条也一样：服务详情 / 历史的链接口径与表格页同一份
    expect(inst['serviceDetailLink']).toBe(ProjectComponent.prototype['serviceDetailLink']);
    expect(inst['serviceHistoryLink']).toBe(ProjectComponent.prototype['serviceHistoryLink']);
  });
   ...
   * 而是因为**跑不到**：`Project1Component` 是独立组件，它自己 `imports` 的 `NzModalModule`
