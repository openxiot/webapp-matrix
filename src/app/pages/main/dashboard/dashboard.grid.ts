 ...
 *
 * ## 让位的口径：被拖的那张钉住，其余的推开再上吸
 *
 * 用户要的是「人眼看上去可以占领的空间，那卡片就可以拖过去占领」，所以**拖到哪就落在哪**
 * （{@link placeAt}），不把它自己吸走 —— 否则「纵向占领空间」这件事根本做不到，
 * 而且拖动中那个落点框会骗人：瞄着一个位置松手却落到别处。
 *
 * 被它压到的卡片**先往下让**，让完再**一路往上吸**（只上吸、不左右挪），于是
 * 「拖到别人身上 → 被压的往下让 → 松手后整屏上吸」这三步是同一个函数里的三行。
 * 横向的留白**保留**：那是用户自己摆的，替他把卡片左右挪等于擅自改版式。
 */
...
/**
 * 把一张卡挪到 `(x, y)`：**它自己钉在那儿**，被压到的先往下让，然后其余整屏上吸。
 *
 * 详见文件头那段让位口径。`x` 会被夹在 `0 .. 24 − w`（拖出右边界时贴着右边停），
 * `y` 夹在 `≥ 0`。找不到这张卡（id 对不上）时原样返回 —— 拖拽与草稿之间短暂错位时不要炸。
 *
...
export function placeAt(items: Placement[], id: string, x: number, y: number): Placement[] {
  ...
  // 让完再上吸，把让位时垫出来的空隙收掉。**钉住的那张不参与**（它就该在用户放的地方）
  return compact(settled, id);
}

/**
 * 上吸：**只往上吸、不左右挪**。`pinned` 那张原地不动（就是刚被拖的那张）。
 *
 * 按 `(y, x)` 序逐张往上顶，顶到「再上一格就会压到已经安顿好的某张」为止。
 * 输出**按 `(y, x)` 重排**，这是幂等的前提（见文件头的不变量 1）。
 */
export function compact(items: Placement[], pinned?: string): Placement[] {
