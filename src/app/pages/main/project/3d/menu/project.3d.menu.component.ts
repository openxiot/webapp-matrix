...
export interface Project3dMenuItem {
  id: string;
  label: string;
  /** 破坏性动作（取消标注）画成红色 */
  danger?: boolean;
 ...
 */
@Component({
  selector: 'project-3d-menu',
  standalone: true,
  templateUrl: './project.3d.menu.component.html',
  styleUrl: './project.3d.menu.component.less',
  changeDetection: ChangeDetectionStrategy.Eager,
})
export class Project3dMenuComponent implements AfterViewInit, OnDestroy {
  /** 相对 `.scene-wrap` 左上角的像素坐标 */
  readonly x = input.required<number>();
  readonly y = input.required<number>();
  readonly items = input.required<Project3dMenuItem[]>();
  /** 标题，如空间名。空则不显示标题区 */
